...
from parkhero.parking.models import ParkingLot
...
class JournalAccount(models.Model):
    ...
    balance = models.IntegerField(
        default=0
    )  # 用户的余额，当为第三方缴费时，可能为0，或者直接使用amount
    spbill_create_ip = models.GenericIPAddressField(
        max_length=50
    )  # 该流水创建时的app用户ip
...
class AliPayLog(models.Model):
    ...
    notify_time = models.CharField(db_index=True, default=None, null=True)
...
class Reconcile(models.Model):
    """ 对账不一致的数据保存在此表 """
    journal = models.ForeignKey(JournalAccount, null=True, default=None,
...
class OfflinePayment(models.Model):
    ...
    payment_type = models.CharField(
        max_length=2, choices=PAYMENT_TYPES, default=PAYMENT
    )
